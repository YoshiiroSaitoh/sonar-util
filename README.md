...
| `projectName.includePrefix` | 任意 | 表示名にも `projectKeyPrefix` を付ける。既定値 `false` |
| `projectName.prefixSeparator` | 任意 | prefixと表示名の区切り。既定値 ` / ` |
...
|---|---|---|
| `services\\api` | `test / services / api` | `test-services-api` |
| `jobs\\api` | `test / jobs / api` | `test-jobs-api` |

`includePrefix=false`にすると表示名から `test / ` を除外できます。`directoryName`を指定すると親階層を表示名へ含めません。Project Keyは表示名設定にかかわらず相対パスを含みます。
