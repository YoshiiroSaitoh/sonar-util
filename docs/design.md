...
`rootPath`はProject候補ではない制御用ルートノードとして表示する。ルートをチェックすると直下ノードをすべてチェックし、その読み込み済み子孫を解除する。ルートを解除すると直下ノードをすべて解除する。したがってルート操作は「直下を個別Projectとして一括選択」の意味であり、rootPath全体を1 Projectにはしない。
